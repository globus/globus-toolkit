...
public class FileTransferProgressServiceDataPanel extends AbstractPortTypePanel implements NotificationSinkCallback {
    ...
    public FileTransferProgressServiceDataPanel() {
	...
	try {
	    ...
	    GSIUtils.setDefaultGSIProperties( (Stub)fileProgress,
            ...
            ExtensibilityType queryResult = fileProgress.findServiceData(QueryHelper.getNameQuery(new QName("","FileTransferProgress")));
            ServiceDataType serviceData = (ServiceDataType) AnyHelper.getAny(queryResult);
            Object[] serviceDataValues = AnyHelper.getAny(serviceData);
            if (serviceDataValues.length < 0) {
                return;
            }
	    Integer transferState = (Integer) serviceDataValues[0];
	} catch(Exception e) {
    ...
    public void addTransferProgressListener() {
        try {
            ...
            this.sink = this.manager.addListener("FileTransferProgress", null, this.handle, this);
        }
        catch (Exception e) {
            e.printStackTrace();
        }
   ...
   public void deliverNotification(ExtensibilityType message) throws RemoteException {
       ...
       Object[] serviceDataValues = AnyHelper.getAny(percentComplete);
       if (serviceDataValues.length < 1) {
           return;
       }
       Integer percentCompleted = (Integer)serviceDataValues[0];
