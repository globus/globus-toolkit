...
import org.globus.ogsa.base.gram.filestreaming.FileStreamingSOAPBindingStub;
import org.globus.ogsa.impl.core.service.ServiceLocator;


public class FileStreamingServiceLocator extends ServiceLocator {
    private final String FileStreamingPort_address = "http://localhost:8080/axis/services";

    public String getFileStreamingPortAddress() {
        return FileStreamingPort_address;
    }

    public FileStreamingPortType getFileStreamingPort()
            throws ServiceException {
        URL endpoint;
        try {
            endpoint = new URL(FileStreamingPort_address);
        }
        ...
        return getFileStreamingPort (endpoint);
    }

    public FileStreamingPortType getFileStreamingPort(URL portAddress)
            throws ServiceException {
        try {

            return new FileStreamingSOAPBindingStub(portAddress,
                                                       this);
        } catch(AxisFault ae) {
            return null;
        }
