...
public class TransferClient {
    ...
    String sourceSubjectName,destinationSubjectName,subjectName;
    ...
    public TransferClient(int transferid,
                          ...
                          RFTOptionsType rftOptions) throws RemoteException {
        try {
            this.transferid = transferid;
            logger.debug("Transfer Params:");
            logger.debug("Binary:"+rftOptions.isBinary() + "BlockSize:"+rftOptions.getBlockSize());
            logger.debug("TcpBufferSize:"+rftOptions.getTcpBufferSize());
            logger.debug("Notpt:"+rftOptions.isNotpt());
            logger.debug("ParallelStreams"+rftOptions.getParallelStreams());
            ...
            if(subjectName != null) {
                destinationHost.setAuthorization(new IdentityAuthorization(subjectName));
                sourceHost.setAuthorization(new IdentityAuthorization(subjectName));
            }
            if(sourceSubjectName != null) {
                sourceHost.setAuthorization(new IdentityAuthorization(sourceSubjectName));
            }
            if(destinationSubjectName != null) {
                destinationHost.setAuthorization(new IdentityAuthorization(destinationSubjectName));
            }
    ...
    private void noTptTransfer() {
        try {
	            ...
	            sink = new FileRandomIO(new java.io.RandomAccessFile(fullLocalFile, 
								 "rw"));
	            sourceHost.extendedGet(sourcePath,
			        size,
			        sink,
			        markerListener);
	            ...
	            source = new FileRandomIO(new java.io.RandomAccessFile(fullLocalFile, 
								   "r"));
	            destinationHost.extendedPut(destinationPath,
			       source,
			       markerListener);
