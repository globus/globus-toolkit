...
*/
void globus_i_io_windows_init_io_operation( 
 ...
 char * buffer, int maxNumberOfBytesToRead, int asynchronous )
{
	...
	handle->winIoOperation.overlapped.Offset= 
	 currentFilePosition.LowPart;
	handle->winIoOperation.overlapped.OffsetHigh= 
	 currentFilePosition.HighPart;

	rc= ReadFile( handle->io_handle, buffer, maxNumberOfBytesToRead,
			NULL, &(handle->winIoOperation.overlapped) );
 ...
 char state )
{
	...
	handle->winIoOperation.state= state;
	handle->winIoOperation.operationAttempted= 0;
	rc= PostQueuedCompletionStatus( completionPort, 0, handle, 
	 &(handle->winIoOperation.overlapped) );
