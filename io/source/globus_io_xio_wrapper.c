...
#define GlobusLIODriverRef(_driver_name)                                    \
    do                                                                      \
    {                                                                       \
        globus_mutex_lock(&globus_l_io_driver_lock);                        \
        {                                                                   \
            if(globus_l_io_##_driver_name##_driver.driver)                  \
            {                                                               \
            ...
            {                                                               \
                globus_xio_driver_load(                                     \
                    #_driver_name,                                          \
                    &globus_l_io_##_driver_name##_driver.driver);           \
                globus_l_io_##_driver_name##_driver.ref_count = 1;          \
            }                                                               \
        }                                                                   \
        globus_mutex_unlock(&globus_l_io_driver_lock);                      \
    } while(0)                                                              
                                                                            
#define GlobusLIODriverFree(_driver_name)                                   \
    do                                                                      \
    {                                                                       \
        globus_mutex_lock(&globus_l_io_driver_lock);                        \
        {                                                                   \
            ...
            {                                                               \
                globus_xio_driver_unload(                                   \
                    globus_l_io_##_driver_name##_driver.driver);            \
                globus_l_io_##_driver_name##_driver.driver = GLOBUS_NULL;   \
            }                                                               \
        }                                                                   \
        globus_mutex_unlock(&globus_l_io_driver_lock);                      \
    } while(0)
