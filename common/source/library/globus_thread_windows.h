...
/* I'm not sure that this is the right place for this one, but we'll leave it here for the
 *  time being.  Michael Lebman 2-7-02 */
#define ETIMEDOUT 1
...
typedef struct TLS_DataDestructionInfo
{
	void * TLS_Data; // a pointer to the data to be destroyed
	globus_thread_key_destructor_func_t destructorFunction; // guess!
} TLS_DataDestructionInfo;

typedef struct globus_i_thread_t
{
	unsigned long threadID;
	UserFunctionInfo userFunctionInfo;
	// list of TLS_DataDestructionInfo objects
	globus_list_t * dataDestructionInfoList;
} globus_i_thread_t;
