    ...
    globus_options_handle_t             handle)
{
        ...
        list = globus_list_rest(list))
    {
        ...
        {
            fprintf(stdout, "-%s | --%s ",
                table->table[i].short_opt, table->table[i].opt_name);
            if(table->table[i].parms_desc != NULL)
            {
                fprintf(stdout, "%s", table->table[i].parms_desc);
            }
            fprintf(stdout, "\n");
            if(table->table[i].description != NULL)
            {
                ndx = 0;
                do 
                {
                    length = strlen(&table->table[i].description[ndx]);
                    if(length > HELP_LINE_LENGTH)
                    {
                        ptr = &table->table[i].description[ndx] 
                            + HELP_LINE_LENGTH;
                        while(ptr > &table->table[i].description[ndx] &&
                            *ptr != ' ')
                        {
                            ptr--;
                        }
                        if(ptr != &table->table[i].description[ndx])
                        {
                            length = ptr - &table->table[i].description[ndx];
                        }
                        else
                        {
                    ...
                    snprintf(buf,
                        length+1,"%s",&table->table[i].description[ndx]);
                    buf[length+1] = '\0';
                    fprintf(stderr, "    %s\n", buf);
                    ndx += length;
                } while(length > 0);
    ...
    char **                             argv)
{
    ...
    for(i = 1; i < argc; i++)
    {
        ...
        if(!found)
        {
            /* call unknow handler if not found and it starts with a -,
                this is so trailing args dont call unknown handler */
            if(handle->unknown_func != NULL && *argv[i] == '-')
            {
                res = handle->unknown_func(argv[i], handle->unknown_arg);
                if(res != GLOBUS_SUCCESS)
                {
                    return res;
                }
