...
// the following definitions are for SASL
...
void send_response_sasl_data(myproxy_socket_attrs_t *attrs, 
			   const char *data, int data_len)
{
    ...
    len = myproxy_serialize_response(&response,
					     buf,
					     sizeof(buf));
...
int recv_response_sasl_data(myproxy_socket_attrs_t *attrs, char *data)
{
   char  buf[SASL_BUFFER_SIZE];
   int   len, result;
