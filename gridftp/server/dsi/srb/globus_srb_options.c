...
{
    int                                 i;
    globus_l_srb_options_handle_t *          handle;

    handle = (globus_l_srb_options_handle_t *)
        globus_calloc(1, sizeof(globus_l_srb_options_handle_t));
    if(handle == NULL)
    {
    }
    ...
    char **                             argv)
{
    ...
    char *                              current_arg;
    globus_bool_t                       found;
    GlobusFuncName(globus_srb_options_command_line_process);
    ...
    for(i = 1; i < argc; i++)
    {
        ...
        if(!found)
        {
            if(strcmp(argv[i],"--help") == 0 ||
                ...
                strcmp(argv[i],"-?") == 0)
            {
                globus_srb_options_help(handle);
                return globus_error_put(globus_error_construct_error(
                    NULL,
                    NULL,
                    GLOBUS_SRB_OPTIONS_HELP,
    ...
    char *                              filename)
{
    ...
    int                                 optlen;
    char *                              p;
    GlobusFuncName(globus_srb_options_file_process);
