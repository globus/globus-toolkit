...
start_client()
{
    ...
    if(pid == 0)
    {
        printf("starting a new backend %s %s\n", l_exe_name, hn);
        rc = execl(l_exe_name, l_exe_name, hn, NULL);
        exit(rc);
    }
    else
    {
        globus_hashtable_insert(&l_host_table, pid, hn);
    }
}
    ...
    xsd_anyType *                       message)
{
    ...
    {
        printf("Connection count was %d now %d.\n", 
    ...
    void *                              userarg)
{
    ...
    if(rc > 0)
    {
        hn = (char *) globus_hashtable_remove(&l_host_table, rc);
        if(hn != NULL)
        {
            globus_fifo_enqueue(&l_host_q, hn);

            printf("backend finished\n");
            rc = WEXITSTATUS(status);
            if(rc != 0)
            {
                printf("####### RETURN CODE ERROR FROM CHILD PROGRAM\n");
                start_client();
            }
        }
    }
