 ...
 *  of the authentication process by setting the start state on the target
 *  attr.
 */
/*
 *  writting
    ...
    void *                                  user_arg)
{
  ...
  err:
    ...
    {
        globus_xio_driver_finished_open(handle->context, handle, op, res);
    }
    return;
}
    ...
    void *                              src)
{
    ...
    res = globus_l_xio_gssapi_ftp_attr_init(&dst_attr);
    if(res != GLOBUS_SUCCESS)
    {
        return res;
    }
