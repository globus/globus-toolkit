...
public class HistogramParser{
    ...
    public void output(PrintStream io){
        ...
        while(iter.hasNext()){
            ...
            while(x<itemNames.length && itemNames[x]!="" && itemNames[x]!=null)
                {
                    if (((ItemEntry)uniqueItems.get(itemName)).get()<((ItemEntry)uniqueItems.get(itemNames[x])).get())
                        {
        ...
        for(int i=0; i<entries.length; i++){
            ...
            io.println(" <entry>");
            io.println("\t<start-date>"+entries[i].getStartDate()+"</start-date>");
            io.println("\t<end-date>"+entries[i].getEndDate()+"</end-date>");
                ...
                while (other.contains(itemNames[itemIndex]))
                    {
                        ...
                        io.println("\t\t\t<single-value>"+entries[i].getData(itemNames[itemIndex])+"</single-value>");
                        io.println("\t\t</sub-item>");
                        otherValue+=entries[i].getData(itemNames[itemIndex]);
                        itemIndex++;
                    }
                io.println("\t\t<single-value>"+(int)otherValue+"</single-value>");
            ...
            while(itemIndex<itemNames.length){
                ...
                io.println("\t\t<single-value>"+(int)value+"</single-value>");
        ...
        if (!rangeSlotted){
                ...
                System.out.println("\t<start-date>"+entries[0].getStartDate()+"</start-date>");
                System.out.println("\t<end-date>"+entries[entries.length-1].getStartDate()+"</end-date>");
                Iterator iter2 = uniqueItems.keySet().iterator();
                while(iter2.hasNext()){
                    System.out.println("\t<item>");
                    itemName = (String)iter2.next();
                    System.out.println("\t\t<name>"+itemName+"</name>");
                    System.out.println("\t\t<single-value>"+((ItemEntry)uniqueItems.get(itemName)).get()+"</single-value>");
                    System.out.println("\t</item>");
                }
                System.out.println(" </slots>");
        }
    ...
    public static class Entry {
        ...
        public void addData(String keyName, double data) {  
            if (itemMap.containsKey(keyName)) {
                itemMap.put(keyName,((ItemEntry)itemMap.get(keyName)).add(data));
            } else{
