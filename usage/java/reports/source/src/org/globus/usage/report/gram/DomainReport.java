...
public class DomainReport{
    ...
    public static void main (String [] args) throws Exception {
        ...
        HistogramParser internalReport = new HistogramParser("Internal MCS/ISI packets recieved", "internalhistogram", "% of Jobs from MCS/ISI", n);
        HistogramParser domainReport = new HistogramParser("Total Jobs Shown by Domain", "domainhistogram", "Number of Jobs", n);
        ...
        while (ts.next()){
            ...
            ResultSet rs = dbr.retrieve("gram_packets",new String [] {"ip_address"}, startTime, ts.getTime());
            while(rs.next()){
                totalJobs++;
            ...
            if (iptracker.getDomains().containsKey("ISI")){
                internalReport.addData("ISI", 100.0*((IPTable.DomainEntry)iptracker.getDomains().get("ISI")).getCount()/totalJobs);
            }
            else{
            ...
            if (iptracker.getDomains().containsKey("MCS")){
                internalReport.addData("MCS", 100.0*((IPTable.DomainEntry)iptracker.getDomains().get("MCS")).getCount()/totalJobs);
            }
            else{
