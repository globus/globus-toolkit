...
public class BufferReport {
    public static void main(String[] args) throws Exception {
        ...
        while (ts.next()) {
            ...
            if (! byteHist.downloadCurrent(dbr)) {
                rs = dbr.retrieve(
            ...
            if (! blockHist.downloadCurrent(dbr)) {
                rs = dbr.retrieve(
                    "SELECT bin.min_rate, COUNT(*) FROM "+
                    "    (SELECT block_size "+
                    "    FROM gftp_packets "+
            ...
            if (! bufferHist.downloadCurrent(dbr)) {
                rs = dbr.retrieve(
                    "SELECT bin.min_rate, COUNT(*) FROM "+
                    "    (SELECT buffer_size "+
                    "    FROM gftp_packets "+
            ...
            if (! bandwidthHist.downloadCurrent(dbr)) {
                rs = dbr.retrieve(
                    "SELECT bin.min_rate, COUNT(*) FROM "+
            ...
            if (! packetHist.downloadCurrent(dbr)) {
                rs = dbr.retrieve(
                        "SELECT COUNT(*) "+
                        "FROM gftp_packets "+
                        "WHERE " +
        ...
        packetHist.upload(dbr);
        byteHist.upload(dbr);
        bandwidthHist.upload(dbr);
        blockHist.upload(dbr);
        bufferHist.upload(dbr);
        dbr.close();
