...
public class BufferReport{
    ...
    public static void main (String [] args) throws Exception{
        ...
        HistogramParser bufferHist = new HistogramParser("TCP Buffer Size", "GFTPtcphistogram", n, "buffer");

        HistogramParser blockHist = new HistogramParser("Block Size", "GFTPblockhistogram", n, "block");

        HistogramParser byteHist = new HistogramParser("Size of Transfer", "GFTPbytehistogram",  n, "byte");

        HistogramParser bandwidthHist = new HistogramParser("Amount of Bandwidth", "GFTPbandwidthhistogram", n, "bandwidth");

        HistogramParser packetHist = new HistogramParser("Transfer Number", "GFTPpackethistogram", "Number of GFTP Transfers", n);
        ...
        while(ts.next()){
            ...
            ResultSet rs = dbr.retrieve("gftp_packets", new String [] {"num_bytes","start_time","end_time","block_size","buffer_size"}, startD, ts.getTime());
            while(rs.next())
                {
                    ...
                    timeDiff = rs.getTimestamp(3).getTime() - rs.getTimestamp(2).getTime();
