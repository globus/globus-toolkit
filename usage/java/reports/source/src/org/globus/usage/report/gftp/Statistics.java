...
public class Statistics {
    ...
    public static void main(String[] args) throws Exception {
        ...
        while (ts.next()) {
            ...
            System.out.println("<start-date>" + ts.getFormattedTime()
                    + "</start-date>");
            ...
            ResultSet rs = dbr.retrieve("gftp_packets", new String[] {
                    "count(*)", "sum(num_bytes)", "sum(block_size)",
                    "sum(buffer_size)" }, startD, ts.getTime());
            rs.next();
            ...
            rs = dbr.retrieve("gftp_packets", new String[] { "num_bytes",
                    "block_size", "buffer_size" }, startD, ts.getTime());
            while (rs.next()) {
            ...
            System.out.println("\t<standard-deviation>"
                    + f.format(bytedeviation) + "</standard-deviation>");
            System.out.println("\t<low-CI>"
                    + f.format((bytemean - (1.96 * bytedeviation)
                            / Math.pow(packetNumber, .5))) + "</low-CI>");
            System.out.println("\t<high-CI>"
                    + f.format((bytemean + (1.96 * bytedeviation)
                            / Math.pow(packetNumber, .5))) + "</high-CI>");
            System.out.println("</byte>");

            System.out.println("<block>");
            System.out.println("\t<mean>" + f.format(blockmean) + "</mean>");
            System.out.println("\t<standard-deviation>"
                    + f.format(blockdeviation) + "</standard-deviation>");
            System.out.println("\t<low-CI>"
                    + f.format((blockmean - (1.96 * blockdeviation)
                            / Math.pow(packetNumber, .5))) + "</low-CI>");
            System.out.println("\t<high-CI>"
                    + f.format((blockmean + (1.96 * blockdeviation)
                            / Math.pow(packetNumber, .5))) + "</high-CI>");
            ...
            System.out.println("\t<standard-deviation>"
                    + f.format(bufferdeviation) + "</standard-deviation>");
            System.out.println("\t<low-CI>"
                    + f.format((buffermean - (1.96 * bufferdeviation)
                            / Math.pow(packetNumber, .5))) + "</low-CI>");
            System.out.println("\t<high-CI>"
                    + f.format((buffermean + (1.96 * bufferdeviation)
                            / Math.pow(packetNumber, .5))) + "</high-CI>");
