			       ...
			       char ** job_contact)
{
    ...
    */
    ...
    contact_msg_size = size;
				/* XXX Should check for NULL return here */
...
/* status module.   User data for globus_io callback. */

/* The globus_gram_client_job_handle_t is just a pointer to this
   structure. */
typedef enum globus_gram_l_async_status_e {	    /* Won't test against all of these
			       states; they're there in case we do
			       use them at some point.   This also
			       serves to document the life of a GRAM
			       asynchronous request. _*/
	L_IDLE = 0, 
	L_INITIATED_CONNECT, 
	L_GOT_CONNECT_CALLBACK, 
	L_INITIATED_WRITE, 
	L_GOT_WRITE_CALLBACK, 
	L_INITIATED_READ, 
	L_GOT_READ_CALLBACK, 
	L_FAILED, 
	L_READ_SUCCEEDED } globus_gram_l_async_status_t;

typedef struct 
globus_gram_l_async_req_status_s {
    volatile enum globus_gram_l_async_status_e state;
    globus_mutex_t mutex;
    globus_cond_t cond;
    int result;	 	  /* error code, if state == L_READWRITE_FAILED */
    char job_contact_str[1000];	/* revolting, isn't it? */
    globus_io_handle_t io_handle;
} globus_gram_l_async_req_status_t;
...
static
void
globus_gram_l_destroy_async_req_status(
    globus_gram_l_async_req_status_t *status)
{
    globus_mutex_destroy(status->mutex);
    globus_cond_destroy(status->cond);
    if (status->io_handle)
	globus_io_handle_free(status->io_handle);
    globus_free(status);
}



/* 0 for good, non-zero for bad parsing. */
/* Convert a resource manager contact string to a form we can pass to 
   globus_io functions. 
   We currently ignore the L_HTTP/L_HTTPS.  Eventually it will just always 
   be HTTPS, but raw HTTP is easier to debug right now. */
/* When this is completed, the arbitrary-length string data returned from 
   it will have to be freed with globus_free().  */
static
int
l_url_to_host_port_jmservicename(const char *url,
				 char **hostp, 
				 unsigned short * portp,
				 char ** jmservicenamep,
				 enum {L_HTTP = 0, L_HTTPS =1 } *securep)
{
    fprintf(stderr, "Need to write a real url parsing function here;"
	    "using dummy data right now.\n"); /* XXX */
    if (hostp)
	*hostp = "vu.isi.edu";
    if (portp) 
	*portp = 2000;
    if (jmservicenamep)
	*jmservicenamep = "/my_job_manager_name";
    if (securep)
	*securep = L_HTTP;
    return GLOBUS_SUCCESS;
    /* Need to add a GLOBUS_GRAM_URL_PARSE_ERROR to the error codes. */
}
...
static 
void
globus_gram_l_async_read_callback(
    void *callback_arg,
    ...
    globus_size_t nbytes)
{
    /* XXX */
    /* Parse the results. */
    fprintf(stderr, "Call the client's callback in order to yield the"
	    " job manager contact string back to" 
	    " the caller; we have succeeded.\n");
    fprintf(stderr, "Must clean up any more allocated structures,"
	    " such as the globus_gram_async_status structure.\n");
    /* XXX */
}
...
static
void
globus_gram_l_async_write_callback(
    void *callback_arg,
    globus_io_handle_t *handle,
    globus_result_t result,
    globus_byte_t *buf,
    globus_size_t nbytes)
{
    fprintf(stderr, "The globus_io WRITE callback was called"
	    " with status %d!\n", result);
    /* XXX */
    fprintf(stderr, "Must set up a globus_io read callback here!\n");
...
/* A pointer to this function is of type globus_io_callback_t */
static
void
globus_gram_l_async_tcp_register_callback(
    void *callback_arg, 
    globus_io_handle_t *handle, 
    globus_result_t result)
{
    /* If we fail, notify the client.  If we succeed, go on and write
       out the data. */
    globus_gram_l_async_req_status_t *status = callback_arg;
    
    /* Fill a buffer with the outgoing data. */
    /* XXX Here we need to now initiate the write callback.
     */
    fprintf(stderr, "Set up a globus_iowrite callback here!\n");
}
 ...
 */
/* SWA notes here:
   resource_manager_contact now has one of two formats:  
   "http://hostname:port/job-manager-service-name" 
   or "https://hostname:port/job-manager-service-name". */
...
{
    int retcode = GLOBUS_SUCCESS;
    /* this will be the user data for the globus_io callback.  */
    globus_gram_l_async_req_status_t *status = NULL; 

    char *host = NULL;		/* host to give to globus_io */
    unsigned short port = 0;	/* port for that host */
    char *job_mgr_servicename;	/* job manager service name. */
    globus_io_attr_t * attr;	/* for the TCP connection */
    globus_bool_t tcpattr_initialized = GLOBUS_FALSE;
    
    handle = NULL;		/* Only set it if successful return. */
    
    /* Initialize status variable. */
    if ((status = globus_malloc(sizeof status)) == NULL) 
    {
	return GLOBUS_GRAM_CLIENT_ERROR_OUT_OF_MEMORY;
    }
    globus_mutex_init(&status.mutex, (globus_mutexattr_t *) NULL);
    globus_cond_init(&status.cond, (globus_condattr_t *) NULL);
    status->state = L_IDLE;
    status->job_contact_str[0] = '\0'; /* unneeded */
    status->io_handle = NULL;

    if(retcode = globus_module_activate(GLOBUS_IO_MODULE)) {
	globus_free(status);
	return retcode;
    }
    GLOBUS_L_LOCK;		/* This should go soon. */

    
    if (retcode = l_url_to_host_port_jmservicename(
	resource_manager_contact, &host, &port, &job_mgr_servicename, NULL)) {
	goto cleanup;
    }    
    /* Initialize ATTR for tcp connection. */
    if (retcode = globus_io_tcpattr_init(&attr))
	goto cleanup;
    tcpattr_initialized = GLOBUS_TRUE;
    /* XXX Here we must add security infrastructure, to make the
       connection SSL-protected.   We also need to engage in the
       initial authentication dialogue with the gatekeeper. */
    fprintf(stderr, "https encryption and authentication to"
	    " be added to this site.\n");
    /* Now dispatch the initial request. */
    if (retcode = 
	globus_io_tcp_register_connect(
	    host, port, attr, globus_gram_l_async_tcp_register_callback, 
	    status, &status->io_handle)) {
	goto cleanup;
    }
    /* The tcp_register_callback will take care of the request from here on. */
    status->state = L_INITIATED_WRITE;
 cleanup:
    GLOBUS_L_UNLOCK;		/* this should go soon.  It is probably
				   unnecessary.  Let's be paranoid. */
    if (tcpattr_initialized == GLOBUS_TRUE) {
	if (!retcode)
	    retcode = globus_io_tcpattr_destroy(&attr);
	else			/*  Keep old error return code */
	    (void) globus_io_tcpattr_destroy(&attr);
    }
    if (retcode) {
	/* If an error */
	/* io_handle_t won't be initialized. */
	if (status)
	    globus_free(status);
	
#if 0 /* XXX Enable the Code here to free the memory allocated by
	 l_url_to_host_port_jmservicename, once that function is no
	 longer returning dummy data. */ 
    if (host) 
	globus_free(host);
    if (job_mgr_servicename)
	globus_free(job_mgr_servicename);
    if (resource_manager_contact)
	globus_free(resource_manager_contact);
#endif
...
{
    fprintf(stderr, "globus_gram_client_job_acknowledge(%s) to be built"
	    "on this spot.\n", job_contact);
}
