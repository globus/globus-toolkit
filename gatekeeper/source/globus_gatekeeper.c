...
#define  GATEKEEPER_RIGHTS          "PROCESS:create"
#define  COND_LOGIN_NAME            "cond_login_name"
#define  COND_AVEREGE_LOAD          "cond_average_load"
#define  PRINCIPALS_REGEX_MATCH     "enable_regex_match" /* should move this to gaa_globus.h */

static
int
gatekeeper_gaa_initialize(gaa_sec_context_ptr *gaa_sc,
                          gaa_rights_ptr      *rights,
                          gaa_data_ptr        *policy_db, 
                          char                *globusid); /* should move this to gaa_globus_utils.c */
...
static
int
gatekeeper_gaa_check_policy_assist(char *globusidp,
                                   char **useridp);

#endif /* #ifdef GAA_GATEKEEPER */
...
static void doit()
{
	...
	*stdout = *fdopen(1,"w");
    ...
    rc = gatekeeper_gaa_check_policy_assist(client_name, &userid);
...
/******************************************************************************
Function:      gatekeeper_gaa_check_policy_assist()
...
static
int
gatekeeper_gaa_check_policy_assist(char *globusidp, char ** useridp)

{
   ...
   gaa_error_code            gaa_result      = GAA_NO;
    ...
    *useridp = NULL;
     ...
     gaa_result = gatekeeper_gaa_initialize(&gaa_sc,
                                            &rights,
                                            &policy_db,
                                            globusidp);


    if(gaa_result != GAA_SUCCESS) 
    ...
    gaa_result = gaa_get_object_policy_info(&minor_status,  
                                             GAA_NO_DATA,
                                             policy_db,
...
fprintf(stderr,"%s\n", policy_db->error_str);
  ...
  gaa_result = gaa_check_authorization (&minor_status,   
                                        gaa_sc,   
                                        policy_handle,     
                                        rights, 
                                        NULL,
 ...
 if(detailed_answer) 
  { 
   ...
   switch(gaa_result)
  {
    case GAA_NO:   result = 3;	/* No entry found in policy file for this user */
                   break;

    case GAA_MAYBE:

    if(gaa_result == GAA_MAYBE)
  {
    gaa_rights_ptr        ptr = detailed_answer->rights;
    gaa_cond_bindings_ptr cond;

     while(ptr) /* checking information in the detailed answer */
    {       
        if(gaa_compare_rights(ptr, rights))
       {       
          cond = ptr->cond_bindings; 

          while(cond && (cond->condition->status != COND_FLG_EVALUATED)) /* check each 
                                             unevaluated condition if there are any */
        {      
          if(!strcmp(cond->condition->type,      COND_LOGIN_NAME)  &&
             !strcmp(cond->condition->authority, AUTH_GLOBUS ) )
	    {
               *useridp = cond->condition->value;
                cond->condition->status |= COND_FLG_EVALUATED; /* evaluated */
                cond->condition->status |= COND_FLG_MET;             /* met */ 
            }
          else 
            { 
             *useridp = NULL;
              
              result = 4; /* this means we see some condition which we do 
                             not understand, it's better to refuse authorization */
              goto cleanup;
	    }

#ifdef EVALUATE_AVEREGE_LOAD

          if(!strcmp(cond->condition->type,      COND_AVEREGE_LOAD) &&
             !strcmp(cond->condition->authority, AUTH_GLOBUS )
            {

                /*  call load evaluation function */
            }
#endif
        
          cond = cond->next; /* check next condition in the detailed answer */  
         }
       }

       ptr=ptr->next; /* check next granted right in the detailed answer */      
  ...
  break;
   ...
   case GAA_YES: result = 5;	/* Sould not get GAA_YES, because local login name
                                   must be returen as a condition. Some error in the
                                   policy file specification */
                 break;

  default: result = 6; /* some other GAA_ERROR */
...
/******************************************************************************
Function:      gaa_gatekeeper_initialize()
...
{ 
 ...
 if(gaa_sc) /* allocate and initiate GAA security context structure */
{
 gaa_principals_ptr principal = NULL;

 gaa_allocate_sec_context(gaa_sc);

 principal =  (*gaa_sc)->identity_cred->principal;
  
  if(strcmp(globus_name, GAA_ANYBODY) == 0) 
    {
      principal->type =      (char *)gaa_strcopy(GAA_ANYBODY, principal->type);
      principal->authority = (char *)gaa_strcopy(" ",         principal->authority);
      principal->value =     (char *)gaa_strcopy(" ",         principal->value);
    }
  else
    {
     principal->type      = (char *)gaa_strcopy(GAA_USER,           principal->type);   
     principal->authority = (char *)gaa_strcopy(GAA_X509_AUTHORITY, principal->authority);    
     principal->value     = (char *)gaa_strcopy(globus_name,             principal->value); 
    }

 }

                    
  if(rights)  /* allocate and initiate access rights structure */
  {
    gaa_allocate_rights(rights);
      
   (*rights)->type       = (char *)gaa_strcopy(POSITIVE_RIGHTS,     (*rights)->type);
   (*rights)->authority  = (char *)gaa_strcopy(AUTH_GLOBUS,         (*rights)->authority);
   (*rights)->value      = (char *)gaa_strcopy(GATEKEEPER_RIGHTS,   (*rights)->value);

  }
    

  if(policy_db) /* allocate gatekeeper policy file path */
   { 
     char *tmp; 
      
     gaa_allocate_data(policy_db);   
     error = gridmap_default_path(&tmp);
     (*policy_db)->str =tmp; 
   }
