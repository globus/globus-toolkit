    ...
    char **                             home_dir_p)
{
    ...
    *home_dir_p = "c:\\windows";
#else
    *home_dir_p = (char *) getenv("HOME");
#endif
...
#else
    uid = getuid();
    *user_id_string = (char *) globus_malloc(32);
    ...
    char **                             cert_dir_p)
{
    char *                              cert_dir;
    char *                              env_cert_dir;
    char *                              default_cert_dir;
    char *                              installed_cert_dir;
    int                                 len;    
#ifdef WIN32
    HKEY                                hkDir = NULL;
    ...
    if (!cert_dir)
    {
        strncpy(val_cert_dir, "<not set>", strlen("<not set>"));
    ...
    if (!cert_dir)
    {
        if (globus_i_gsi_checkstat(X509_INSTALLED_HOST_CERT_DIR) == 0)
        {
            ...
            len = strlen(X509_INSTALLED_HOST_CERT_DIR);
            cert_dir = (char *) globus_malloc(sizeof(char) * len);
            if(!cert_dir)
            {
                result = GLOBUS_GSI_SYSTEM_CONFIG_MALLOC_ERROR;
    ...
    char **                             user_key_p)
{
    ...
    {
        len = strlen(env_user_cert);
        user_cert = (char *) globus_realloc ((void *) user_cert, 
                                             sizeof(char) * len);
        if(!user_cert)
        {
            result = GLOBUS_GSI_SYSTEM_CONFIG_MALLOC_ERROR;
    ...
    if(!user_cert)
    {
        strncpy(val_user_cert, "<not set>", strlen("<not set>"));
    ...
    if(!user_key)
    {
        strncpy(val_user_key, "<not set>", strlen("<not set>"));
    ...
    char **                             host_key_p)
{
    ...
    {
        len = strlen(env_host_cert);
        host_cert = (char *) globus_realloc ((void *) host_cert,
                                             sizeof(char) * len);
        if(!host_cert)
        {
            result = GLOBUS_GSI_SYSTEM_CONFIG_MALLOC_ERROR;
    ...
    if(!host_cert)
    {
        strncpy(val_host_cert, "<not set>", strlen("<not set>"));
    ...
    if(!host_key)
    {
        strncpy(val_host_key, "<not set>", strlen("<not set>"));
    ...
    if(((result = globus_i_gsi_get_home_dir(&home)) == GLOBUS_SUCCESS)
       && !host_cert)
    {
        len = strlen(X509_DEFAULT_HOST_CERT) + 2;
    ...
    {
        len = strlen(X509_DEFAULT_HOST_KEY) + 2;
    ...
    if(!host_cert || !host_key)
    {
        ...
        len = strlen(X509_DEFAULT_HOST_PKCS12_FILE) + 2;
    ...
    int                                 proxy_in)
{
    ...
    {
        RegOpenKey(HKEY_CURRENT_USER,GSI_REGISTRY_DIR,&hkDir);
        strncpy(val_user_proxy, "<not set>", strlen("<not set>"));
