 ...
 */
int call_server(host, port, oid, service_name, deleg_flag, use_encrypt)
     char *host;
     u_short port;
     gss_OID oid;
     char *service_name;
     ...
     int use_encrypt;
{
	 ...
	 maj_stat = gss_wrap(&min_stat, context, use_encrypt, GSS_C_QOP_DEFAULT,
	 ...
	 printf("%s", (out_buf.value ? out_buf.value : "(null)\n"));
     }
