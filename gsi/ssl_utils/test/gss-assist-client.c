...
void usage()
{
     fprintf(stderr, "Usage: gss-assist-client [-port port] [-d] host service \
msg\n");
     exit(1);
}
     ...
     OM_uint32 *ret_flags;
{
     ...
     * to do the GSSAPI authentication to the server
     * we will use the assist functions.
	 * As a test of the assist routines, we wil use the
	 * nexus version, but pass it a socket. We can 
	 * simulate the nexus socket routines in this test
	 * module. 
     */

	maj_stat = globus_gss_assist_init_sec_context(
					&min_stat,
			        credential_handle,
                    gss_context,
                    service_name,
    ...
    if (maj_stat != GSS_S_COMPLETE)
    {
        globus_gss_assist_display_status(stderr,
                    "GSS failure:gss_assist_init_sec_context",
 ...
 */
int call_server(host, port, oid, service_name, deleg_flag, msg, use_file, use_encrypt)
     char *host;
     u_short port;
     gss_OID oid;
     char *service_name;
	 ...
	 int use_encrypt;
{
     ...
     if (use_file) {
		...
		char * cp;
	 	in_buf.length = strlen(msg) + 2;
		cp = in_buf.value = (char *) malloc(in_buf.length);
	 	memcpy(in_buf.value, msg, in_buf.length);
	 	cp[in_buf.length-2] = 13;
	 	cp[in_buf.length-1] = 10;
     }

	 qop_state = GSS_C_QOP_DEFAULT;
     maj_stat = gss_wrap(&min_stat, context, use_encrypt, qop_state,
     ...
     /* Send to server */
#if 1
	 if( globus_gss_assist_token_send_nexus_without_length(&s, 
			out_buf.value, out_buf.length) != 0) 
#else
...
#if 1
	/* read back a response, which might be encrypted 
	 * There may be multiple buffers, so read till eof */

	 while ((rc = globus_gss_assist_token_get_nexus(&s,
			&in_buf.value, &in_buf.length)) == 0) 
	 {
	
		fprintf(stderr,"inbuf len=%d\n",in_buf.length);
	 	maj_stat = gss_unwrap(&min_stat,context, 
						&in_buf, &out_buf,&conf_state, NULL);
    	if (maj_stat != GSS_S_COMPLETE || out_buf.length == 0)
    	{
			break;
		}
		write(1,out_buf.value, out_buf.length);

     	(void) gss_release_buffer(&min_stat, &out_buf);
	 }
	/* catch token and unwrap problems here */
	/* could be the EOF which we need to test for seperatly */
   	if (maj_stat != GSS_S_COMPLETE)
   	{
      	 globus_gss_assist_display_status(stderr,
      	         "Unwrap loop",
     ...
     char **argv;
{
     ...
     int use_file = 0;
	 int use_encrypt = 0;
     OM_uint32 deleg_flag = 0 ,maj_stat, min_stat;
    ...
    {
        globus_gss_assist_display_status(stderr,
                "gss_assist_acquire_cred failure",
                maj_stat,
                min_stat,
                0);
		exit (1);
