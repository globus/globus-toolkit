...
#define SEGFakeEnter() \
        SEGFakeDebug(SEG_FAKE_DEBUG_INFO, ("Enter %s\n", _globus_func_name))

#define SEGFakeExit() \
        SEGFakeDebug(SEG_FAKE_DEBUG_INFO, ("Exit %s\n", _globus_func_name))

/**
 * Debug levels:
 * If the environment variable SEGFakeDebug is set to a bitwise or
...
typedef enum
{
     ...
     */
    SEG_FAKE_DEBUG_INFO = (1<<0),
    /**
     * Warnings of things which may be bad.
     */
    SEG_FAKE_DEBUG_WARN = (1<<1),
    /**
     * Fatal errors.
     */
    SEG_FAKE_DEBUG_ERROR = (1<<2),
    /**
     * Details of function executions.
     */
    SEG_FAKE_DEBUG_TRACE = (1<<3)
}
globus_l_seg_fake_debug_level_t;

#ifdef BUILD_DEBUG
#define SEGFakeDebug(level, message) \
    GlobusDebugPrintf(SEG_FAKE, level, ("%s", globus_l_seg_fake_level_string(level))); \
    GlobusDebugPrintf(SEG_FAKE, level, message)
#else
#define SEGFakeDebug(level, message) \
    if (level == SEG_FAKE_DEBUG_ERROR) \
    { \
        fprintf(stderr, "%s", globus_l_seg_fake_level_string(level)); \
        globus_l_seg_fake_debug message; \
    }
static
void
globus_l_seg_fake_debug(const char * fmt, ...)
{
    va_list ap;
...
static
char *
globus_l_seg_fake_level_string(globus_l_seg_fake_debug_level_t level)
{
    switch (level)
    {
        case SEG_FAKE_DEBUG_INFO:
            return "[INFO] ";
        case SEG_FAKE_DEBUG_WARN:
            return "[WARN] ";
        case SEG_FAKE_DEBUG_ERROR:
            return "[ERROR] ";
        case SEG_FAKE_DEBUG_TRACE:
            return "[TRACE] ";
        default:
            return "";
    }
...
enum
{
    SEG_FAKE_ERROR_UNKNOWN = 1,
    SEG_FAKE_ERROR_OUT_OF_MEMORY,
    SEG_FAKE_ERROR_BAD_PATH,
    SEG_FAKE_ERROR_LOG_PERMISSIONS,
    SEG_FAKE_ERROR_LOG_NOT_PRESENT,
    SEG_FAKE_ERROR_PARSE
...
} globus_l_fake_logfile_state_t;

static globus_mutex_t                   globus_l_fake_mutex;
static globus_cond_t                    globus_l_fake_cond;
static globus_bool_t                    shutdown_called;
static int                              callback_count;


GlobusDebugDefine(SEG_FAKE);
...
static
int
globus_l_fake_parse_events(
    globus_l_fake_logfile_state_t *     state);

static
int
globus_l_fake_clean_buffer(
    globus_l_fake_logfile_state_t *     state);

static
int
globus_l_fake_increase_buffer(
    globus_l_fake_logfile_state_t *     state);

static
int
globus_l_fake_find_logfile(
    globus_l_fake_logfile_state_t *     state);
...
globus_l_fake_module_activate(void)
{
    globus_l_fake_logfile_state_t *     logfile_state;
    int                                 rc;
    globus_reltime_t                    delay;
    globus_result_t                     result;
    int                                 save_errno;
    GlobusFuncName(globus_l_fake_module_activate);
    ...
    if (globus_module_getenv("SEG_FAKE_DEBUG") == NULL)
    {
        globus_module_setenv("SEG_FAKE_DEBUG", "ERROR");
    }
    GlobusDebugInit(SEG_FAKE, INFO WARN ERROR TRACE);

    SEGFakeEnter();

    rc = globus_mutex_init(&globus_l_fake_mutex, NULL);

    if (rc != GLOBUS_SUCCESS)
    {
        SEGFakeDebug(SEG_FAKE_DEBUG_ERROR,
                ("Fatal error initializing mutex\n"));
        goto deactivate_common_error;
    }
    rc = globus_cond_init(&globus_l_fake_cond, NULL);
    if (rc != GLOBUS_SUCCESS)
    {
        SEGFakeDebug(SEG_FAKE_DEBUG_ERROR,
    ...
    logfile_state = globus_libc_calloc(
            1,
            sizeof(globus_l_fake_logfile_state_t));
    ...
    rc = globus_l_fake_increase_buffer(logfile_state);
    if (rc != GLOBUS_SUCCESS)
    {
        save_errno = errno;
        SEGFakeDebug(SEG_FAKE_DEBUG_ERROR,
    ...
    rc = globus_l_fake_find_logfile(logfile_state);
    ...
    if (rc == GLOBUS_SUCCESS)
    {
        ...
        {
            rc = SEG_FAKE_ERROR_OUT_OF_MEMORY;
...
destroy_cond_error:
    globus_cond_destroy(&globus_l_fake_cond);
destroy_mutex_error:
    globus_mutex_destroy(&globus_l_fake_mutex);
...
globus_l_fake_module_deactivate(void)
{
    GlobusFuncName(globus_l_fake_module_deactivate);

    SEGFakeEnter();

    globus_mutex_lock(&globus_l_fake_mutex);
    shutdown_called = GLOBUS_TRUE;

    while (callback_count > 0)
    {
        globus_cond_wait(&globus_l_fake_cond, &globus_l_fake_mutex);
    }
    globus_mutex_unlock(&globus_l_fake_mutex);

    SEGFakeExit();
    GlobusDebugDestroy(SEG_FAKE);
...
{
    int                                 rc;
    globus_l_fake_logfile_state_t *     state = user_arg;
    size_t                              max_to_read;
    globus_bool_t                       eof_hit = GLOBUS_FALSE;
    globus_reltime_t                    delay;
    globus_result_t                     result;
    GlobusFuncName(globus_l_fake_read_callback);
    ...
    if (shutdown_called)
    {
        SEGFakeDebug(SEG_FAKE_DEBUG_INFO,
    ...
    if (state->fp != NULL)
    {
        ...
        SEGFakeDebug(SEG_FAKE_DEBUG_TRACE,
        ...
        if (rc < max_to_read)
        {
            if (feof(state->fp))
            {
                SEGFakeDebug(SEG_FAKE_DEBUG_TRACE, ("hit eof\n"));
                eof_hit = GLOBUS_TRUE;
                clearerr(state->fp);
            }
            else
        ...
        /* Parse data */
        SEGFakeDebug(SEG_FAKE_DEBUG_TRACE, ("parsing events\n"));
        rc = globus_l_fake_parse_events(state);

        SEGFakeDebug(SEG_FAKE_DEBUG_TRACE, ("cleaning buffer\n"));
        rc = globus_l_fake_clean_buffer(state);
    ...
    {
        rc = globus_l_fake_find_logfile(state);
        if(rc == SEG_FAKE_ERROR_LOG_NOT_PRESENT)
        {
...
error:
    ...
    if (shutdown_called)
    {
        ...
        if (callback_count == 0)
        {
            globus_cond_signal(&globus_l_fake_cond);
        }
    }
    globus_mutex_unlock(&globus_l_fake_mutex);
...
/**
 * Determine the next available FAKE log file name from the 
 * timestamp stored in the logfile state structure.
 * 
 * @param state
 *     FAKE log state structure. The path field of the structure may be
    ...
    globus_l_fake_logfile_state_t *     state)
{
    ...
    int                                 rc;
    int                                 save_errno;
    GlobusFuncName(globus_l_fake_find_logfile);

    SEGFakeEnter();

    if (state->path == NULL)
    {
        SEGFakeDebug(SEG_FAKE_DEBUG_TRACE, ("allocating path\n"));

        globus_common_get_attribute_from_config_file(NULL,
                "etc/globus-fake.conf", "log_path", &state->path);
    ...
    do
    {
        ...
        if (rc < 0)
        {
            ...
            {
                case ENOENT:
                    SEGFakeDebug(SEG_FAKE_DEBUG_WARN,
                        ("missing log file\n"));
                    rc = SEG_FAKE_ERROR_LOG_NOT_PRESENT;
                    goto error;
                ...
                case EACCES:
                    ...
                    rc = SEG_FAKE_ERROR_LOG_PERMISSIONS;
                    goto error;
...
error:
    if (state->path == NULL)
    {
        SEGFakeDebug(SEG_FAKE_DEBUG_ERROR,
                ...
                "$GLOBUS_LOCATION/etc/globus-fake.conf\n"));
    }
    else
    {
        SEGFakeDebug(SEG_FAKE_DEBUG_ERROR,
...
{
    int                                 rc = 0;
    GlobusFuncName(globus_l_fake_clean_buffer);
    ...
    if (state->buffer != NULL)
    {
        ...
        rc = globus_l_fake_increase_buffer(state);
    }
    SEGFakeExit();
    return rc;
}
 ...
 *
 * @param state
 *     FAKE log state structure. The buffer-related fields of the structure
    ...
    globus_l_fake_logfile_state_t *     state)
{
    ...
    const size_t                        GLOBUS_FAKE_READ_BUFFER_SIZE = 4096;
    int                                 rc;
    GlobusFuncName(globus_l_fake_increase_buffer);
    ...
    if (state->buffer_valid == state->buffer_length)
    {
        state->buffer = globus_libc_realloc(state->buffer,
                    state->buffer_length + GLOBUS_FAKE_READ_BUFFER_SIZE);
        if (state->buffer == NULL)
        {
            SEGFakeDebug(SEG_FAKE_DEBUG_ERROR, ("realloc() failed\n"));
    ...
    globus_l_fake_logfile_state_t *     state)
{
    ...
    int                                 jobid_start;
    int                                 jobid_end;
    GlobusFuncName(globus_l_fake_parse_events);
    enum {
