...
class BarrierServer(BaseHTTPServer.HTTPServer):
    ...
    def barrier(self, barrier_num, job_id, objects):
        if barrier_num not in self.barrier_data:
            self.barrier_data[barrier_num] = {}

        if len(self.barrier_data[barrier_num]) >= self.barrier_size:
            return (409, "Conflict")
        elif job_id in self.barrier_data[barrier_num]:
            return (409, "Conflict")

        self.barrier_data[barrier_num][job_id] = objects
        return (202, "Accepted")

    def get_barrier_data(self, barrier_num):
        barrier_data = self.barrier_data.get(barrier_num)
...
class BarrierRequest(BaseHTTPServer.BaseHTTPRequestHandler):
    ...
    barrier_get_re = re.compile("/barrier/([0-9]+)")
    ...
    def do_POST(self):
        ...
        if m is not None:
            ...
            code, reason = self.server.barrier(barrier_num, job_id, objects)
    ...
    def do_GET(self):
        ...
        if m is not None:
            barrier_num = int(m.group(1))
            code, reason, data = self.server.get_barrier_data(barrier_num)
            self.send_response(code, reason)
            encoded = None
            if code == 200:
